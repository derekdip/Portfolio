...
const Settings: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({className,style}) => {
    ...
    const handleClipboardCopy = () => {
        navigator.clipboard.writeText(JSON.stringify(selectedPalette))
    }
    ...
    return(
        ...
        <Modal show={show} onHide={handleClose}>
            ...
            <H1 style={{color: selectedPalette.primary}}>Settings</H1>
            </Modal.Header>
            <Modal.Body>
                <Container>
                <Row>
                <Col>
                    <Form.Group >
                        <ColorInput title="Primary Color" startingColor={selectedPalette.primary} setColor={(color:string)=>{
                ...
                <Col>
                    <Form.Group >
                        <ColorInput title="Secondary Color" startingColor={selectedPalette.secondary} setColor={(color:string)=>{
                ...
                <Col>
                    <Form.Group >
                        <ColorInput title="Third Color" startingColor={selectedPalette.accent1} setColor={(color:string)=>{
                                setLocalPalette({...localPalette, accent1:parseColorInput(color)})
                            }}/>
                    </Form.Group>
                    </Col>
                    <Col>
                    <Form.Group >
                        <ColorInput title="Fourth Color" startingColor={selectedPalette.accent2} setColor={(color:string)=>{
                                setLocalPalette({...localPalette, accent2:parseColorInput(color)})
                            }}/>
                    </Form.Group>
                    </Col>
                </Row>
                <Row>
                    <Col>
                    <Form.Group >
                        <ColorInput title="Background Color" startingColor={selectedPalette.background} setColor={(color:string)=>{
            ...
            <Modal.Footer>
            <Button variant="secondary" onClick={handleClipboardCopy}>
                Copy to Clipboard
            </Button>
            <Button variant="secondary" onClick={handleGlobalColorChange}>
                Change Colors
            </Button>
